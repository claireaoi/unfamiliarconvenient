...
#To check also tag type word (adjective etc) but without context. As the check with context may be misleading.
...
#   txtChris= ' '.join(re.findall("[a-zA-Z\-\.'/]+", f.read())) #Text with list words
...
with open('./data/roo.txt') as f:
   rawRoo= ''.join(f.readlines()) #Text with list words
   #REMOVE DOTS!>>


print(lemmatizer.lemmatize("developers"))
print(wordnet.synsets("developers"))
print(lemmatizer.lemmatize("others"))
print(wordnet.synsets("others"))
print(lemmatizer.lemmatize("robots"))
print(wordnet.synsets("robot"))
...
def nonLetters(inputString):
    ...
    for char in inputString:
        if char.isalpha():
            count+=1
    return len(inputString)- count

#print("nb letter:", nonLetters(rawChris))
...
disambiguationPage("Prospect_theory")
                ...
                if not word is None and len(word)>1 and not hasNumbers(word) and (pos == 'NN' or pos == 'NNS' or pos == 'NNP'):
                    ...
                    for tmp in wordnet.synsets(word): #Take all tags corresponding to exactly the name
                        if tmp.name().split('.')[0] == word:
                            pos2.append(tmp.pos())
                    #Check if disambiguation page: with ppprop  of a page<page _idx="2400147" pageid="2400147" ns="0" title="BNI"> <pageprops disambiguation=""/>
                    #Or in title written "title": "word (disambiguation)", but not always
                    #query.search[1].title==word+ "_(disambiguation)"
                    #Check if disambiguation page ??
                    if wikipedia.page(word).exists() and (pos == 'NNP' or (len(pos2)>0 and pos2[0]=='n')) and not (word in OKWikipedia) and not word in graph.keys() and not word.lower() in graph.keys() and not word.lower() in memory:#up to capital:
        ...
        for i, word in enumerate(wordList):
            if counter<nbWord:
                ...
                if i<len(wordList)-1:
                    ...
                    duo=word+" "+nextword
                    if len(word)>1 and len(nextword)>1 and word not in excluded and nextword not in excluded and not hasNumbers(word) and not hasNumbers(nextword) and wikipedia.page(duo).exists() and not (duo in OKWikipedia):
...
#OKWikipedia,OKWiktionary=extract(rawChris, dict(), [], 8)
#OKWikipedia2,OKWiktionary2=extract(rawRoo, dict(), [], 8)

# Computing English word similarity using Li method
#TEST Semantic similarity >>>

#score=semanticSimilarity('wet', 'east side')
#score=semanticSimilarity('cat', 'east side')
