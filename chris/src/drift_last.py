...
####selfGraph is a dictionnary, whose keys are concepts, and values are couple (weight, neighbofrs).
...
print("Self is Here.")
print("Initial Concepts within Self:", selfGraph.keys())
memory=selfGraph.keys() #Init memory. Memory is all word has looked for ? Forget sometimes, as when grow network, could be actually connected with it again..>>>
...
def MLDrift(seedSentence):
    drift= cs.cond_model(model_name='124M',seed=None, nsamples=1, batch_size=1,length=lengthML,temperature=1,top_k=0,top_p=1, models_dir='./chris/models', blabla = seedSentence)
...
def selfAwarenessQuest(word):
    ...
    for wordSelf in selfGraph.keys():
...
def oneCycle(question="Christopher, tell me about acid rain.", nLoop=2,  nDrift=2):
